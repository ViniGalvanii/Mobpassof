...
export function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try { const data = await signIn(email, senha); login(data.token); navigate("/map"); }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-violet-700 via-purple-800 to-violet-900">
      <div className="flex-1 flex flex-col items-center justify-center px-6 pt-16 pb-8">
        <div className="mb-8"><Logo /><p className="text-center text-purple-200 text-sm mt-2">Acompanhe rotas em tempo real</p></div>
        <div className="w-full bg-white rounded-3xl p-6 shadow-2xl shadow-purple-900/50">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-2"><Button type="submit" disabled={loading}>{loading?"Entrando...":"Entrar"}</Button></div>
          </form>
